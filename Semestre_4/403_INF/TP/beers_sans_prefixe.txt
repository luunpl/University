...
CREATE TABLE Beers (
  name TEXT NOT NULL, --NOT NULL pas nécessaire pour un attribut clé dans certains SGBD
  manufacturer TEXT,
  CONSTRAINT beers_c0 PRIMARY KEY (name)
);
...
CREATE TABLE Bars (
	...
	licence DATE,
	opendate DATE,
	CONSTRAINT bars_c0 PRIMARY KEY (name)
);
CREATE TABLE Drinkers (
	name TEXT NOT NULL, 
	address TEXT, 
	phone TEXT,
	CONSTRAINT drinkers_C0 PRIMARY KEY (name)
);
CREATE TABLE Frequents (
	drinker TEXT NOT NULL, 
	bar TEXT NOT NULL,
	CONSTRAINT frequents_c0 PRIMARY KEY (drinker, bar),
	CONSTRAINT frequents_c1 FOREIGN KEY (bar) REFERENCES Bars(name),
	CONSTRAINT frequents_c2 FOREIGN KEY (drinker) REFERENCES Drinkers(name)
);

CREATE TABLE Likes(
  drinker TEXT NOT NULL,
  beer TEXT NOT NULL,
  CONSTRAINT likes_c0 PRIMARY KEY (drinker,beer),
  CONSTRAINT likes_c1 FOREIGN KEY (beer) REFERENCES Beers(name),
  CONSTRAINT likes_c2 FOREIGN KEY (drinker) REFERENCES Drinkers(name)
);
...
CREATE TABLE Sells(
  ...
  beer TEXT NOT NULL,
  price REAL NOT NULL,
  CONSTRAINT sells_c0 PRIMARY KEY (bar,beer),
  CONSTRAINT sells_c1 FOREIGN KEY (bar) REFERENCES Bars(name),
  CONSTRAINT sells_c2 FOREIGN KEY (beer) REFERENCES Beers(name),
  CONSTRAINT sells_c3 CHECK (price > 0)
);
