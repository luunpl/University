 ...
 * Renvoie Vrai si la suppression a été effectuée, et Faux si la valeur
 * n'est pas présente dans l'arbre.
 * Attention : il est important de conserver la propriété d'ABR de l'arbre :
 * tout sous-arbre gauche ne doit contenir que des valeurs inférieures à la 
 * racine, et tout sous-arbre droit des valeurs supérieures.
 * */
bool supprimer_abr(arbre *a, int n) {
    /* à compléter */
    return false;
}
